...
public:
	...
	//function of the checking the all room status that if it is empty or not
	void all_RoomStatus() {
		Room* temp1 = new Room();
		...
		while (!q_Room.empty()) {
			...
			if (temp1->check == 1) {
				cout << "Room # " << temp1->room_no << " \t\tFULL" << endl;
			}
			else {
				cout << "Room # " << temp1->room_no << " \t\tEMPTY" << endl;
			}
			s_Room.push(*temp1);
			q_Room.pop();
		}
		while (!s_Room.empty()) {
			temp11->air_conditioner = s_Room.top().air_conditioner;
			temp11->attached_bath = s_Room.top().attached_bath;
			temp11->check = s_Room.top().check;
			temp11->room_no = s_Room.top().room_no;
			temp11->room_size; s_Room.top().room_size;
			temp11->room_status; s_Room.top().room_status;
			s_Room_s.push(*temp11);
			s_Room.pop();
		}
		while (!s_Room_s.empty()) {
			temp2->air_conditioner = s_Room_s.top().air_conditioner;
			temp2->attached_bath = s_Room_s.top().attached_bath;
			temp2->check = s_Room_s.top().check;
			temp2->room_no = s_Room_s.top().room_no;
			temp2->room_size; s_Room_s.top().room_size;
			temp2->room_status; s_Room_s.top().room_status;
			q_Room.push(*temp2);
			s_Room_s.pop();
		}
	}
...
void for_Admin() {
	...
	cout << "\n\t\t6.Press 6 to Insert the room details  ";
	cout << "\n\t\t7.Press 7 to Delete any room details  ";
	cout << "\n\t\t8.Enter 0 to exit                    ";
...
int main() {
	...
	Room* head = NULL;
	//Admin admin;
	insertRoom(&head, 1, "Double", true, true, true, true);
	insertRoom(&head, 2, "Single", true, true, true, false);
	insertRoom(&head, 3, "Double", true, true, false, true);
	insertRoom(&head, 4, "Single", true, false, true, true);
	insertRoom(&head, 5, "Single", false, true, true, true);
	/*	display();
		deleteRoom(3);
		cout << endl<<endl;
		display();
